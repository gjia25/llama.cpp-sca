...
int main(int argc, char ** argv) {
    ...
    while (std::getline(infile, line)){
        ...
        if (inp.size() > n_batch){
            LOG_ERR("%s : expected batch size < %d but got %d\n", __func__, n_batch, inp.size());
            continue;
        }
        batch_add_seq(batch, inp, 0);
        batch_decode(ctx, batch);
        
        t_main_end = ggml_time_us();
        fprintf(stderr, "%s: encoded %d tokens in %.2f s, speed: %.2f t/s\n",
                __func__, batch.n_tokens, (t_main_end - t_main_start) / 1000000.0f, batch.n_tokens / ((t_main_end - t_main_start) / 1000000.0f));
        t_main_start = ggml_time_us();
